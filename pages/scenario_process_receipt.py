...
from template_streamlit.agents.receipt_processor.agent import receipt_workflow
from template_streamlit.agents.receipt_processor.models import (
    AccountInfo,
    CommandType,
    DisplayMode,
    EventType,
...
from template_streamlit.agents.receipt_processor.storage import get_saved_receipts
from template_streamlit.agents.receipt_processor.ui_components import (
    account_info_editor,
...
def handle_feedback_submission() -> Feedback | None:
    """
    ユーザーのフィードバックを収集して返す
    ...
    # アクションボタンを表示
    feedback: Feedback | None = display_action_buttons()

    return feedback


def process_workflow(
    input_data: str | Command,
    ...
    with st.spinner(spinner_message):
        try:
            # ワークフローの実行
            stream_iterator = receipt_workflow.stream(input_data, config=config, stream_mode=["custom", "values"])
            ...
            for mode, payload in stream_iterator:
                ...
                if mode == "custom":
                    ...
                    if event == EventType.OCR_DONE:
                        ...
                        ocr_result = ReceiptOCRResult.model_validate(payload["structured_data"])
                        st.session_state.ocr_text = raw_text
                        st.session_state.ocr_result = ocr_result

                    elif event == EventType.ACCOUNT_SUGGESTED:
                        account_info = AccountInfo.model_validate(payload["account_info"])
                        ...
                        st.session_state.error_message = f"未知のイベントを受信しました: {event}"
                    ...
                    if "__interrupt__" in payload:
                        ...
                        ocr_result = ReceiptOCRResult.model_validate(interrupt.value.get("ocr_result"))
                        account_info = AccountInfo.model_validate(interrupt.value.get("account_info"))
                        st.session_state.ocr_result = ocr_result
                        st.session_state.account_info = account_info
            ...
            st.session_state.error_message = f"ワークフロー実行中にエラーが発生しました: {e}"
            st.session_state.workflow_state = WorkflowState.ERROR
            st.rerun()
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("新規領収書処理", use_container_width=True, disabled=new_receipt_disabled):
    ...
    with left_col:
        ...
        # 処理開始ボタン - 処理中は無効化
        start_disabled = not image_path or st.session_state.workflow_state != WorkflowState.IDLE
    ...
    with right_col:
        ...
        if st.session_state.workflow_state == WorkflowState.WAIT_FEEDBACK:
            ...
            feedback: Feedback | None = handle_feedback_submission()
            if feedback:
        ...
        # エラー表示
        if st.session_state.workflow_state == WorkflowState.ERROR and hasattr(st.session_state, "error_message"):
