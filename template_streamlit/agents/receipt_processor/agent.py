...
from template_streamlit.agents.receipt_processor.account import suggest_account_info
from template_streamlit.agents.receipt_processor.constants import CSV_FILE_PATH
...
from template_streamlit.agents.receipt_processor.storage import backup_csv, save_to_csv
from template_streamlit.agents.receipt_processor.vision import ocr_receipt
...
def generate_account_suggestion(
    ...
    *,
    ...
    if feedback_history and len(feedback_history) > 0:
        # すべてのフィードバックを結合して渡す
        combined_feedback = " ".join([f"フィードバック{i + 1}: {fb}" for i, fb in enumerate(feedback_history)])
            ...
            writer(
                {
                    ...
                    "message": f"不正なコマンドです: {feedback.command}。'approve'または'regenerate'を使用してください。",  # noqa
